...
def task_9_8():
    ...
    for i in range(len(input_phrase) // 2):
        res += input_phrase[i]
    print(res)

    tmp = ''
    
    for i in range(len(input_phrase) - 1, len(input_phrase) // 2 - 1, - 1):
        tmp += input_phrase[i]
    print(tmp, '\n')
    ...
    if len(output_phrase) % 2 == 0:
        corrective = 1

    print(f"Зашифрованное слово: {output_phrase[0:len(output_phrase):2] + output_phrase[len(output_phrase) - 2 + corrective:0:-2]}",
        '\n')
